...
description: >
  ...
  A* vs Dijkstra 哪个快), offer a hands-on experiment card. Keywords: 卡片 card
  看板 dashboard widget 可视化 visualize. Skip it for coding, debugging, git,
  translation, formatting, and one-to-two sentence factual answers.
allowed-tools:
...
| **RICH** | background / scheduled / subagent run (nobody watching) **or** user said 精致 / 完整 / 详细 / 报告 | ≤25KB | full anatomy, multi-chart, full motion |
| **LAB** | "取决于参数/场景" questions — propose-first (see **LAB**) | free, stay lean anyway | experiment stage |
| **SERVE** | a running task to watch live (tests/build/deploy/scrape) — offer-first; shell renders before data, fills via SSE, settles to static (see **SERVE**) | free, stay lean anyway | skeleton-first; relay server + live stream |
...
1. **Data first.** Run every search / fetch / computation and finalize all numbers before you touch any HTML — the card embeds data, so until it's complete there's nothing to write. For content digests (news/posts/papers), per-item substance is *part of* "data complete": full headline + 2–4句 summary + key quote/figure + source/time. LAB: this step instead defines the experiment variables + their controls.
2. **Map shape → layout + chart** via the tables below (first match wins).
3. **Announce + read.** One line on what's coming + a soft ETA range, rounded up (FAST 1–2分钟 · DASH 2组件 2–4分钟 · RICH/完整DASH 4–6分钟); finishing early beats overrunning. Then **Read `SNIPPETS.md`** — and in the same message `CHARTS.md` if the card draws any chart or uses DASH, `COLLECTIONS.md` if it's a content collection (parallel reads, one round-trip).
4. **Write.** The next tool call after the Read is the Write. Dense CSS (one rule per line, shorthands, only rules you use); FAST shell lands ~8–10KB.
5. Open it (interactive: `open <file>`) or report the path (background).
...
| 5+ measures of ONE entity (a dashboard) | **STAGE + graphic-selector** (SNIPPETS §Stage switcher) — one big interactive graphic + a strip of mini-graphic tabs to switch metric. Dense yet uncluttered; a number-grid here is the banned failure mode. ONE accent for the whole card. | SNIPPETS |
| several genuinely independent KPI groups | **DASH** widget bento — ONE accent across ALL widgets (one-hue-per-widget reads as a rainbow), each widget graphic-first + interactive. FAST → 2 richest widgets (~10KB) + upgrade offer; RICH → full 2×2 | CHARTS |
...
| ranked / leaderboard | **featured spotlight + ranked bars** (CHARTS §Leaderboard) — a selected-entity spotlight (its trend spark + share + delta) over leader-relative bars; tap any row to feature it. Champion = the one loud accent bar, runners muted. | CHARTS |
...
| ordered procedure (≥3 dependent steps, each carrying rich content) | **STEPPER** (SNIPPETS); <3 steps or label-only → plain timeline, don't upgrade | — |
| entities | icon tile rows | — |
| single long article | editorial exemption | — |

**Chart by relationship** (inline SVG, accent pair, fixed pixel height; geometry in CHARTS):

| Relationship | Chart |
|--------------|-------|
| trend over time | line + same-hue area fade + end dot + value tag pill |
| value vs target | bullet / ring + dashed target line + label pill |
| categories ≤8 | rounded bars, values on bars; gray all + highlight ONE in accent + tag |
| avg / benchmark | dashed line + ink label pill |
| share (≤5 slices) | donut / concentric 2-ring |
...
| sequence / events | single-SVG spine + dots + fractional label grid (never a CSS-absolute line behind flex dots) |
| change | delta chip (SVG triangle + %) |
| dense vital / stream | spike micro-bars + peak anchor annotation |
| phase / segment | strip chart |
| multi-dim profile (5–8 axes, ≤2 series) | radar (fixed-px only) |
...
- **Citations** — sources never on the card face: one quiet affordance (evidence button / faint `来源 ×n`). Claims carry tiny accent-tinted `.ref` indices opening the overlay; inside, numbered entries + one compact SVG row of letter-circle source marks (SNIPPETS).
...
- **LIVE channel**: clicks in a rendered card wake the agent (drill-back / multi-round LAB) — interactive sessions only, never to ask a question. Preflight + listener mechanism in **`LAB-LIVE.md`**. (Card→agent; the inverse of SERVE.)
- **SERVE mode**: a running task to watch live (test progress, build, deploy, scrape) — the card's shell renders before data exists and fills as the task runs, then settles into an ordinary static card. Offer-first; agent→card over SSE; degrades to static when no server. Trigger, lifecycle, schema, and the copy-exact relay server + card boot snippets live in **`SERVE.md`** — load it before building one.

## Output

Write to a `ViewCard/` folder in cwd — path `ViewCard/<topic>-card.html` (create the folder if absent: `mkdir -p ViewCard`); keeps generated cards collected, not scattered in cwd. Interactive: `open ViewCard/<topic>-card.html 2>/dev/null || true`. Background/subagent/scheduled: do NOT open, report the path. Follow-ups about a rendered card → answer in HTML (shell 2–10KB, detail in overlay); plain text only for 1–2 sentence answers. Tweaks to an existing card (swap a chart, recolor, add/remove rows, reword) → surgical Edit calls — seconds, not a regenerate; full rewrite only for layout-level restructuring.
**Multi-card index** — so consecutive 整理资料 don't scatter into N HTMLs: `mkdir -p ViewCard` also copies the frozen shell `assets/index-shell.html` → `ViewCard/index.html` (copy-exact, `[ -f ] ||` never overwrites). Each data card embeds a `<script id="card-meta">` block and appends one copied line to the shell's `CARDS` marker — a file-authoritative iframe navigator (search + tag + time-group + overview grid, `file://`-offline, no server). Interactive open = `open "file://$(pwd)/ViewCard/index.html#<card-file>"` (a `file://` absolute URL — a bare relative path with `#` makes macOS `open` seek a literal `index.html#…` file and silently fail), not the single card. Procedure + field schema → read **`MULTI-CARD.md`** (load before building a collection). SERVE live-task cards stay single-file.
